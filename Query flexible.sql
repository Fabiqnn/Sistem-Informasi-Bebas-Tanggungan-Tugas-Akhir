...
CREATE TABLE FORM_PUSTAKA (
	...
	FILE_PENDAHULUAN varchar(255) not null,
...
SELECT * FROM FORM_TA join MAHASISWA on FORM_TA.NIM = MAHASISWA.NIM LEFT JOIN VERIFIKASI on FORM_TA.ID_VERIFIKASI = VERIFIKASI.ID_VERIFIKASI WHERE MAHASISWA.NIM = '2341720170';
...
delete from FORM_TA where niM = '2341720170'
delete from FORM_PUSTAKA where niM = '2341720170'
delete from FORM_TA where niM = '2341720170'
update FORM_PRODI
...
ALTER TABLE [FORM_PUSTAKA]
alter column IZIN_MENGOLAH varchar(30) not null
...
INSERT INTO [USER] (ROLE, PASS, ID_USER) values 
('adm_lt7', '1234567', '1907234576')

delete from [USER] where ID_USER = '1907234576'
...
CREATE VIEW ViewPrintBebasTanggungan AS
SELECT 
    ft.NIM,
    vta.STATUS_VERIFIKASI AS STATUS_TA,
    vp.STATUS_VERIFIKASI AS STATUS_PRODI,
    vpust.STATUS_VERIFIKASI AS STATUS_PUSTAKA
FROM FORM_TA ft
LEFT JOIN VERIFIKASI vta ON ft.ID_VERIFIKASI = vta.ID_VERIFIKASI
LEFT JOIN FORM_PRODI fp ON ft.NIM = fp.NIM
LEFT JOIN VERIFIKASI vp ON fp.ID_VERIFIKASI = vp.ID_VERIFIKASI
LEFT JOIN FORM_PUSTAKA fpust ON ft.NIM = fpust.NIM
LEFT JOIN VERIFIKASI vpust ON fpust.ID_VERIFIKASI = vpust.ID_VERIFIKASI;

SELECT * FROM ViewPrintBebasTanggungan;
